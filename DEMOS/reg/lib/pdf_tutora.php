...
$sql = "SELECT id_publico, nombre, a_paterno, a_materno, soy, matricula, institucion.nombre_ins, c_personal, medio, taller, fecha, solucion, tipo, material, aviso, costo, status FROM publico INNER JOIN institucion ON publico.procedencia = institucion.id_institucion WHERE id_publico =  '$registro'";
$registros = mysql_query($sql);
$fila = mysql_fetch_array($registros);
$nombre = $fila['nombre'];
$procedencia = $fila['nombre_ins'];
$costo = $fila['costo'];
       ...
       $pdf->Cell(0,5,"Estimado(a): ".$nombre,0,1,'L');
       $pdf->Cell(0,5,utf8_decode("Procedencia: ".$procedencia),0,1,'L');
