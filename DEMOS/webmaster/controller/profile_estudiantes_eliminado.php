<table class="table table-condensed" id="tabla_folios" style="border-collapse:collapse;">
    ...
    <tbody>
<?php
    while($reg=  mysql_fetch_array($estudiantes_eliminado)){
    ...
    echo '<tr>';
    echo '<td> <span class="badge"><i class="fa fa-ticket"></i> FNEP'.date('Y').'-'.mb_convert_encoding($reg['id_basura'], "UTF-8").'</span></td>';
    ...
    echo '<td class="visible-lg">'.mb_convert_encoding($reg['nombres'], "UTF-8").'</td>';
}
?>
    </tbody>
</table>